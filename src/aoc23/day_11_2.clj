   ...
   (let [lines (vec lines)
         ...
         measure-distance (fn [[[row1 col1] [row2 col2]]]
                            (let [[col1 col2] (if (<= col1 col2) [col1 col2] [col2 col1])
                                  naive-dist (+ (- row2 row1) (- col2 col1))
                                  ...
                                  add-dists (* (+ add-row-dists add-col-dists) (dec expansion))]
                              (+ naive-dist add-dists)))
         galaxy-distances (map measure-distance galaxy-combinations)]
     (reduce + galaxy-distances)))
  ([lines] (solve 1000000 lines)))
...
(deftest solve-test
  (let [universe ["...#......"
                  ...
                  ".......#.."
                  "#...#....."]]
    (is (= (solve 2 universe) 374))
    (is (= (solve 10 universe) 1030))
    (is (= (solve 100 universe) 8410))))
